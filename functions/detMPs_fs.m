function [fint,Kt,mpData] = detMPs_fs(uvw,mpData)
...
parfor mp=1:nmp                                                                % material point loop
    ...
    elseif mpData(mp).cmType == 3
        [D,Ksig,epsE]=exactPR(mpData(mp).epsEn,epsEtr,mpData(mp).mCst(1),mpData(mp).mCst(2),mpData(mp).mCst(3));                      % elasto-plastic behaviour (von Mises)
    end
